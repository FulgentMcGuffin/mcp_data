...
``read_only=True`` (the default, used by the MCP server) the connection is
...
def _resolve_default_db_path() -> tuple[str, str]:
    """Resolve ``(directory, filename)`` for the default DB from ``MCP_DB_PATH``.

    Evaluated lazily (never at import time) so importing this module never
    requires ``MCP_DB_PATH`` to be configured.

    Raises:
        ValueError: if ``MCP_DB_PATH`` is unset or does not end in ``.db``.
    """
    raw = os.getenv("MCP_DB_PATH")
    if raw is None:
        raise ValueError("MCP_DB_PATH environment variable is not set")
    name = os.path.basename(raw)
    if not name.endswith(".db"):
        raise ValueError(
            f"MCP_DB_PATH environment variable must end with .db but found {name}"
        )
...
class SQLiteSource(DataSink):
    ...
    Implements the read-only :class:`DataBackend` protocol (consumed by the MCP
    ...
    Args:
        db_path: Path to the SQLite file, ``":memory:"`` for an in-memory db, or
            ``None`` to derive the location from the ``MCP_DB_PATH`` env var.
    ...
    # ------------------------------------------------------------------
    # Default-path helpers (derived from MCP_DB_PATH, resolved lazily)
    # ------------------------------------------------------------------

    @classmethod
    def default_db_dir(cls) -> str:
        """Directory portion of ``MCP_DB_PATH``."""
        return _resolve_default_db_path()[0]

    @classmethod
    def default_db_name(cls) -> str:
        """Filename portion of ``MCP_DB_PATH``."""
