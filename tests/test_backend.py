...
def test_list_tables(backend: SQLiteSource) -> None:
    assert backend.list_tables() == ["customers", "orders"]


def test_get_schema(backend: SQLiteSource) -> None:
...
def test_get_schema_unknown_table(backend: SQLiteSource) -> None:
    with pytest.raises(QueryError):
        backend.get_schema("does_not_exist")


def test_run_query_returns_polars(backend: SQLiteSource) -> None:
    df = backend.run_query("select * from customers")
    assert isinstance(df, pl.DataFrame)
    assert df.height == 5


def test_run_query_empty_result_keeps_columns(backend: SQLiteSource) -> None:
...
def test_run_query_rejects_non_readonly(backend: SQLiteSource, sql: str) -> None:
    with pytest.raises(QueryError):
